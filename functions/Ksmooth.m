...
% Initialize output matrices
  [m, nobs] = size(S.Zm);
  S.ZmT = zeros(m,nobs+1); % Smoothed factors
  S.VmT = zeros(m,m,nobs+1); % Smoothed variances
  ...
  % Loop through time reverse-chronologically (starting at final period nobs)
    for t = nobs:-1:1
        ...
        VmU = squeeze(S.VmU(:,:,t)); % P_t|t
        Vm1 = squeeze(S.Vm(:,:,t)); % P_t|t-1
        
        % Store previous period smoothed factor covariance and lag-1 covariance
        V_T = squeeze(S.VmT(:,:,t+1)); % P_t|T
        V_T1 = squeeze(S.VmT_1(:,:,t)); % P_t-1|T for EM algorithm
